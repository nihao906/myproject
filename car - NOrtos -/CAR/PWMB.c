...
void PWMB_init(void)
{
 //1、使能GPIO时钟 PF9
  ...
  GPIO_Init(GPIOF,&GPIO_INSTRUCT);
	...
	//4、将GPIO复用为定时器  
	GPIO_PinAFConfig(GPIOF,GPIO_PinSource9,GPIO_AF_TIM14);//将PF9复用为TIM14
	...
	TIM_TIMINSTRUCT.TIM_Prescaler = 1 - 1;//100HZ

	//7、定时器初始化
	TIM_TimeBaseInit(TIM14,&TIM_TIMINSTRUCT);
  ...
  TIM_OC1PreloadConfig(TIM14, TIM_OCPreload_Enable);//使能定时器通道1预加载，主要是用来产生连续的PWM
  TIM_ARRPreloadConfig(TIM14, ENABLE);//使能ARR寄存器预加载
