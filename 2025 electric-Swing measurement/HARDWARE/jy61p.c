...
void Ajy61p_ReceiveData(uint8_t RxData)
{
	...
	else if (RxState == 2)	//接收数据
	{
		...
		if(RxIndex == 11)	//接收完成
		{
			for(i=0;i<10;i++)
			{
				sum = sum + ARxBuffer[i]; //计算校验和
			}
			if(sum == ARxBuffer[10])		//校验成功
			{
				/*计算数据，根据数据内容选择对应的计算公式*/
				Ax = ((uint16_t) ((uint16_t) ARxBuffer[3] << 8 | (uint16_t) ARxBuffer[2])) / 32768.0f * 16.0f;
				Ay = ((uint16_t) ((uint16_t) ARxBuffer[5] << 8 | (uint16_t) ARxBuffer[4])) / 32768.0f * 16.0f;
				Az = ((uint16_t) ((uint16_t) ARxBuffer[7] << 8 | (uint16_t) ARxBuffer[6])) / 32768.0f * 16.0f;
			}
			RxState = 0;
			RxIndex = 0; //读取完成，回到最初状态，等待包头
		}
