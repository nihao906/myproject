...
//华大GNSS配置
/*如果是本身内部支持华大GNSS的型号，这里不要做任何改动！！！
如果是客户自己外挂华大GNSS，客户需要将对应型号的moudle_def值改为HD_GNSS_CUSTOM，
并且将剩下的其他配置项需要根据自己的硬件设计配置一致，包括串口号、串口脚以及控制脚*/
volatile ql_hd_gnss_func_s hd_gnss_ctx = {
#if defined(CONFIG_QL_PROJECT_DEF_EC800G) || defined(CONFIG_QL_PROJECT_DEF_EG800G)
    .moudle_def = HD_GNSS_800G,     //客户自己外挂HD GNSS时需要改为HD_GNSS_CUSTOM
    .uart_port = QL_UART_PORT_5,    //内置用的UART5，客户自己外挂HD GNSS时需要修改
    //以下都是客户自己外挂的情况自己配置的
    .tx_pin = 86,
    .tx_func = 2,
    .rx_pin = 83,
    .rx_func = 2,
    .rst_pin = 23,                  //复位脚
    .rst_func = 1,                  //复位脚GPIO FUNC
    .rst_gpio = 14,
    .boot_pin = 85,                 //boot脚
    .boot_func = 1,                 //boot脚GPIO FUNC
    .boot_gpio = 10, 
#elif defined CONFIG_QL_PROJECT_DEF_EC200G
    .moudle_def = HD_GNSS_200G,     //客户自己外挂HD GNSS时需要改为HD_GNSS_CUSTOM
    .uart_port = QL_UART_PORT_6,    //内置用的UART5，客户自己外挂HD GNSS时需要修改
    //以下都是客户自己外挂的情况需要自己配置的
    .tx_pin = 82,
    .tx_func = 2,
    .rx_pin = 81,
    .rx_func = 2,
    ...
    .boot_gpio = 10,
#elif defined CONFIG_QL_PROJECT_DEF_EG915G
    .moudle_def = HD_GNSS_915G,
    .uart_port = QL_UART_PORT_6,
#endif
};
#endif
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BT
#if (CONFIG_QUEC_PROJECT_FEATURE_BT_BLE_PACK == 1)
	{  HAL_POWER_CAMA,	    true,	  false,	   POWER_LEVEL_UNUSED,		POWER_INIT_PHASE2  },	/* CAMA */      /* not available */
#else
    {  HAL_POWER_CAMA,	    true,	  false,	   POWER_LEVEL_3100MV,		POWER_INIT_PHASE2  },	/* CAMA */      /* not available */
#endif
#else
