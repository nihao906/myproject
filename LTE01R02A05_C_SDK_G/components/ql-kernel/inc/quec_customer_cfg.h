...
typedef struct
{
    ql_hd_gnss_moudle_e moudle_def;    //HD GNSS适配型号，0为客户自己外挂
    ql_uart_port_number_e uart_port;   //华大GNSS串口
    uint8_t tx_pin;
    uint8_t tx_func;
    uint8_t rx_pin;
    uint8_t rx_func;
    uint8_t rst_pin;                   //复位脚
    uint8_t rst_func;                  //复位脚GPIO FUNC
    uint8_t rst_gpio;                  //复位脚GPIO NUM
    uint8_t boot_pin;                  //boot脚
    uint8_t boot_func;                 //boot脚GPIO FUNC
    uint8_t boot_gpio;                 //boot脚GPIO NUM
} ql_hd_gnss_func_s;
