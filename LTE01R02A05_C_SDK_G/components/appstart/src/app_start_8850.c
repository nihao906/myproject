...
//if defined CONFIG_APP_FLASH2_ENABLED or CONFIG_APPIMG_FLASH2_ENABLED,it cannot skip mount "/ext" here because APPIMG may be loaded in file mode
    ...
    {
        OSI_LOGI(0x1000897d, "restart from blue screen");
...
//open版本一般不需要data engine,除了蓝牙相关的部分AT以及文件系统AT用到
    ...
    //提前GNSS初始化，避免产线工具很早就发AT开启GNSS报错
...
/*开机阶段直接更新GNSS固件,客户可以更改固件更新动作的时间点
可以放在app更新，也可以直接屏蔽这里不更新GNSS固件，但是注意
开启GNSS的动作需要在固件更新完成后*/
    ql_gnss_firmware_update();
//开机自启动或者固件更新阶段有收到AT+QGPS=1开启GNSS
    extern bool updated_reset;
    if(updated_reset == true)
    {
        quec_gnss_power_onff(GNSS_ENABLE);
    }
#endif
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BT
/*开机阶段直接更新BT固件,客户可以更改固件更新动作的时间点
可以放在app更新，也可以直接屏蔽这里不更新BT固件，但是注意
开启BT的动作需要在固件更新完成后*/
    ql_bt_firmware_update();
#endif
