...
#define SPI_CAM_MCLK    15000000
#define MIPI_CAM_MCLK   26000000
...
bool camGc0310Open(void)
{
    ...
    prvCamGc0310PowerOn();
#ifdef CONFIG_QUEC_PROJECT_FEATURE_CAMERA
	// mipi camera mclk seems to have a minimum frequency requirement!
	// If the mipi camera mclk is less than 24M, the data cannot be received normally.
	// mipi camera default mclk:26Mhz;spi camera default mclk:15Mhz
	ql_cam_mclk_freq = (p->sensorType == camCsi_In) ? MIPI_CAM_MCLK : SPI_CAM_MCLK;
#endif
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_CAMERA
	//如果需要增大驱动能力的,可以打开此部分代码
#if 0
	//bit(20:19):'drv' control for normal mode 0: Driven strength 2mA 1: Driven strength 4mA 2: Driven strength 6mA 3: Driven strength 8mA
	//the default driver level is 2,here we adjust it to 3.
#define _DRV 3
	hwp_iomux->pad_camera_rst_l = (hwp_iomux->pad_camera_rst_l & (~(_DRV<<19))) | (_DRV<<19);
	hwp_iomux->pad_spi_camera_sck = (hwp_iomux->pad_spi_camera_sck & (~(_DRV<<19))) | (_DRV<<19);
	hwp_iomux->pad_spi_camera_si_1 = (hwp_iomux->pad_spi_camera_si_1 & (~(_DRV<<19))) | (_DRV<<19);
	hwp_iomux->pad_spi_camera_si_0 = (hwp_iomux->pad_spi_camera_si_0 & (~(_DRV<<19))) | (_DRV<<19);
#endif
#endif
