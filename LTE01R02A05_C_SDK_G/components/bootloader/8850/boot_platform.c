...
#ifdef CONFIG_QUEC_PROJECT_FEATURE
#ifndef CONFIG_CHARGER_POWERUP
//修复USB在位关机后，查询RTC时间错误问题 JIRA:STUNISOC8850-1452 CQ:SPCSS01289385
    uint16_t epoch_l = 0;
    uint16_t epoch_h = 0;
    ...
    if (psm_magic == OSI_SHUTDOWN_POWER_OFF)
    {
         epoch_l = halAdiBusRead(&PSM_EPOCH_LOW) & 0xffff;
         epoch_h = halAdiBusRead(&PSM_EPOCH_HIGH) & 0xffff;
    }
    // disable PSM module
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE
#ifndef CONFIG_CHARGER_POWERUP
//修复USB在位关机后，开机查询RTC时间错误问题 JIRA:STUNISOC8850-1452 CQ:SPCSS01289385
    if (psm_magic == OSI_SHUTDOWN_POWER_OFF)
    {
        halAdiBusWrite(&PSM_MODE_REG, psm_magic);

        halAdiBusBatchChange(&PSM_EPOCH_HIGH,
                             HAL_ADI_BUS_OVERWITE(epoch_h),
                             &PSM_EPOCH_LOW,
                             HAL_ADI_BUS_OVERWITE(epoch_l),
