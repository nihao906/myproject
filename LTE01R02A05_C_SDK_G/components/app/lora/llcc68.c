...
#include "ql_log.h"
#define LOGI(msg, ...)  QL_LOG(QL_LOG_LEVEL_INFO, "lora_llcc68", msg, ##__VA_ARGS__)
...
void LLCC68Init(void)
{
    LLCC68Reset( );//复位引脚先低电平输出，然后上拉输入(低电平100us复位)

    //LLCC68IoIrqInit( dioIrq );

    LLCC68Wakeup( );//write RADIO_GET_STATUS cmd
    LLCC68SetStandby( STDBY_RC ); //设置为rc震荡

    // Initialize TCXO control
    //LLCC68IoTcxoInit( );

    // Initialize RF switch control
    LLCC68IoRfSwitchInit( ); //write RADIO_SET_RFSWITCHMODE cmd

    LLCC68SetOperatingMode( MODE_STDBY_RC );
}

int LLCC68CheckDeviceReady( void )
{
    QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68CheckDeviceReady start");
    int mode = LLCC68GetOperatingMode();
    QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","mode = %d\n",mode);
    int flag = -1;
    //LOGI("LLCC68GetOperatingMode end\n");
    if(mode == 0 || mode == 6 )
    {
        QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","mode is MODE_SLEEP or MODE_RX_DC");

        flag = LLCC68Wakeup();
        QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","flag = %d\n",flag);
        if(flag == -1){
            //LOGI("LLCC68Wakeup fail\n");
            QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68Wakeup fail\n");
            return flag;
        }else{
            QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68Wakeup ok\n");
        }
    ...
    if(flag == -1){
        ...
        QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68WaitOnBusy fail\n");
        //return -1;
    }else{
        QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68WaitOnBusy ok\n");
    }

    QL_LOG(QL_LOG_LEVEL_INFO,"in_LLCC68CheckDeviceReady","LLCC68CheckDeviceReady end, flag = %d\n",flag);
    return flag;
}
...
int LLCC68SetStandby( RadioStandbyModes_t standbyConfig )
{
    QL_LOG(QL_LOG_LEVEL_INFO,"inLLCC68SetStandby","LLCC68SetStandby start");
    int flag = -1;
    flag = LLCC68WriteCommand( RADIO_SET_STANDBY, &standbyConfig, 1 );
    if(flag == -1){
        QL_LOG(QL_LOG_LEVEL_INFO,"inLLCC68SetStandby","LLCC68SetStandby -> LLCC68WriteCommand fail");
        return -1;
    }
    
    //LOGI("LLCC68WriteCommand end\n");
    ...
    {
        flag = LLCC68SetOperatingMode( MODE_STDBY_XOSC );
    }
    QL_LOG(QL_LOG_LEVEL_INFO,"inLLCC68SetStandby","LLCC68SetStandby end");
    return flag;
}
...
int LLCC68SetRegulatorMode( RadioRegulatorMode_t mode )
{
    //LOGI("enter LLCC68SetRegulatorMode\n");
    return LLCC68WriteCommand( RADIO_SET_REGULATORMODE, ( uint8_t* )&mode, 1 );
}
...
int LLCC68SetPaConfig( uint8_t paDutyCycle, uint8_t hpMax, uint8_t deviceSel, uint8_t paLut )
{
    uint8_t buf[4];
    ...
    buf[2] = deviceSel;
    buf[3] = paLut;
    return LLCC68WriteCommand( RADIO_SET_PACONFIG, buf, 4 );
}
...
int LLCC68SetDioIrqParams( uint16_t irqMask, uint16_t dio1Mask, uint16_t dio2Mask, uint16_t dio3Mask )
{
    uint8_t buf[8];
    ...
    return LLCC68WriteCommand( RADIO_CFG_DIOIRQ, buf, 8 );
}

uint16_t LLCC68GetIrqStatus( void )
{
    //uint8_t irqStatus[2] = {0x00,0x00};
    rx_buf[0] = 0x00;
    rx_buf[1] = 0x00;
    LLCC68ReadCommand( RADIO_GET_IRQSTATUS, rx_buf, 2 );
...
int LLCC68SetTxParams( int8_t power, RadioRampTimes_t rampTime )
{
    uint8_t buf[2];
    int flag = -1;
    ...
    //{
        ...
        flag = LLCC68WriteRegister( 0x08D8, LLCC68ReadRegister( 0x08D8 ) | ( 0x0F << 1 ) );
        // WORKAROUND END

        flag = LLCC68SetPaConfig( 0x04, 0x07, 0x00, 0x01 );
        if( power > 22 )
        {
            power = 22;
        }
    ...
    flag = LLCC68WriteCommand( RADIO_SET_TXPARAMS, buf, 2 );
    return flag;
}
...
int LLCC68SetBufferBaseAddress( uint8_t txBaseAddress, uint8_t rxBaseAddress )
{
    uint8_t buf[2];

    buf[0] = txBaseAddress;
    buf[1] = rxBaseAddress;
    return LLCC68WriteCommand( RADIO_SET_BUFFERBASEADDRESS, buf, 2 );
}
...
RadioStatus_t LLCC68GetStatus( void )
{
    ...
    LLCC68ReadCommand( RADIO_GET_STATUS, rx_buf, 0 );
    stat = rx_buf[0];
