...
char url[128] = "http://49.235.127.120/output.pack";
...
int fota_http_init(fota_http_client_t* fota_http_cli_p)
{
	...
	fota_http_cli_p->i_save_size = 0;
	memcpy(fota_http_cli_p->fota_packname, FOTA_SDMMC_FILENAME, strlen(FOTA_SDMMC_FILENAME));	//默认升级包地址
...
*  other 	-- failed 
************************************************************/
extern int check_data_call(void);
static int fota_http_evn_request(fota_http_client_t* fota_http_cli_p)
{
	...
	//注网拨号
	QL_FOTA_HTTP_LOG("check data call...");
	if (check_data_call())
	{
		QL_FOTA_HTTP_LOG("data call failed");
		fota_http_close_fd(fota_http_cli_p);
		return -1;
	}
	QL_FOTA_HTTP_LOG("data call is ok");
	...
	{
		QL_FOTA_HTTP_LOG("http perform err");
	}
	ql_httpc_release(&(fota_http_cli_p->http_cli));
	fota_http_close_fd(fota_http_cli_p);
	return -3;
...
void ql_fota_http_app_init(char *fileurl)
{
	strcpy(url, fileurl);
	QL_FOTA_HTTP_LOG("ql_fota_http_app_init(%s)", url);
