...
static void ql_ledcfg_thread(void *param)
{
    ...
    // int ret = ql_pin_set_func(QUEC_PIN_DNAME_GPIO_22, 0);   //gpio22 gpio
    // if(QL_GPIO_SUCCESS != ret)
    // {
    //    QL_LEDCFGDEMO_LOG("ql_pin_set_func gpio:%d ret: 0x%x", QUEC_PIN_DNAME_GPIO_22,ret);
    // }
    // ql_gpio_deinit(QL_RS485_GPIO_NUM);
    // ql_gpio_init(QUEC_PIN_DNAME_GPIO_22, GPIO_OUTPUT, QUEC_PIN_NONE, LVL_LOW);
    while(1)
    {
#if 0
/***** demo1 *****/
...
void ql_ledcfg_app_init(void)
{
    ...
    err = ql_rtos_task_create(&ledcfg_task, 1024, APP_PRIORITY_NORMAL, "ql_ledcfgdemo", ql_ledcfg_thread, NULL, 1);
