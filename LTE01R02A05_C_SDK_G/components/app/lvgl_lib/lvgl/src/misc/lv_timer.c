...
#define LOG 0
#define     LOG_INFO(msg, ...)          QL_LOG(QL_LOG_LEVEL_INFO, "lvtimer", msg, ##__VA_ARGS__)
/**********************
 *      TYPEDEFS
 **********************/
...
LV_ATTRIBUTE_TIMER_HANDLER uint32_t lv_timer_handler(void)
{
    TIMER_TRACE("begin");
#if LOG
    LOG_INFO("lv_timer_handler start");
#endif
...
#if LOG
    LOG_INFO("handler_start = %d",handler_start);
#endif
...
#if LOG
    LOG_INFO("Run all timer from the list");
#endif

    /*找到下一个即将触发的定时器，并计算从当前时间到下一个定时器触发所需的时间*/
...
#if LOG
    LOG_INFO("time_till_next = %d",time_till_next);
#endif

    /*计算系统的空闲时间和忙碌时间，并根据此计算系统的利用率*/
...
#if LOG
    LOG_INFO("lv_timer_exec start");
#endif

    if(timer->paused) return false;

#if LOG
    LOG_INFO("timer not paused");
#endif
...
#if LOG
        LOG_INFO("lv_timer_time_remaining(timer) == 0");
#endif
...
#if LOG
        LOG_INFO("calling timer callback: %p",*((void **)&timer->timer_cb));
#endif
        if(timer->timer_cb && original_repeat_count != 0) timer->timer_cb(timer);
        //TIMER_TRACE("timer callback %p finished", *((void **)&timer->timer_cb));
#if LOG
        LOG_INFO("timer callback %p finished", *((void **)&timer->timer_cb));
#endif
...
#if LOG
            LOG_INFO("deleting timer with %p callback because the repeat count is over", *((void **)&timer->timer_cb));
#endif
            lv_timer_del(timer);
        }
    }
