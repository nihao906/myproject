...
//  senser_cfg_t *senser_cfg = (senser_cfg_t *)gWordVar[REG_CFG_SERSER];
...
void rs485_task(void *param)
{
  ...
  // senser_cfg->temp_type = 1;
  // senser_cfg->temp_count = 1;
  // load_sensor_cfg(senser_cfg);
  ...
  while (1)
  {
    ...
    // rx_len = rs485_read_bytes(uart_rxbuf, 256, 100);
    // if (rx_len > 0)
    // {
      // if (senser_cfg->temp_type == 1)
      // {
      ...
      //   }
      // }
      // else if (senser_cfg->temp_type > 1)
      // {
      //   uint16_t crc16_cal = crc16(uart_rxbuf, rx_len-2);
      //   uint16_t crc16_rx = (uart_rxbuf[rx_len-2] << 8) | uart_rxbuf[rx_len-1];
      //   if (crc16_cal == crc16_rx && uart_rxbuf[1] == 0x03)
      //   {
      //     if (senser_cfg->temp_type == 2)
      //     {
      //       int16_t temp = (uart_rxbuf[3] << 8) | uart_rxbuf[4];
      //       uint8_t addr = uart_rxbuf[0];
      //       LOGD("addr:%d,temp:%d\n", addr,temp);
      //       int16_t max_temp = get_temp_max(temp);
      //       gWordVar[REG_TEMPS + addr - 1] = max_temp;
      //     }
      //     else if (senser_cfg->temp_type == 3)
      //     {
      //       uint16_t vibrate = (uart_rxbuf[3] << 8) | uart_rxbuf[4];
      //       gWordVar[REG_VIBRATE_SENSOR] = vibrate;
      //       LOGD("vibrate %d\n", 1, vibrate);
      //     }
      //   }
      //   else
      //   {
      //     LOGD("bad response crc16_cal:%04X crc16_rx:%04X\n", crc16_cal, crc16_rx);
      //   }
      //   uart_flag = 0;
      // }
