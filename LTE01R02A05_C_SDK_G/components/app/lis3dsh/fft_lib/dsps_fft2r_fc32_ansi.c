...
#include "inc/dsps_fft2r.h"
#include "inc/dsp_common.h"
#include "inc/dsp_types.h"
#include <math.h>
#include "inc/esp_attr.h"
#include "inc/esp_log.h"
#include <string.h>
#include <malloc.h>
...
esp_err_t dsps_gen_bitrev2r_table(int N, int step, char *name_ext)
{
    ...
    LOGI("const uint16_t bitrev2r_table_%i_%s[] = {        ", N, name_ext);
    for (int i = 1; i < (N - 1); i++) {
        k = N >> 1;
        while (k <= j) {
            j -= k;
    ...
    LOGI("};");
    LOGI("const uint16_t bitrev2r_table_%i_%s_size = %i;\n", N, name_ext, items_count);

    LOGI("extern const uint16_t bitrev2r_table_%i_%s[];", N, name_ext);
    LOGI("extern const uint16_t bitrev2r_table_%i_%s_size;\n", N, name_ext);
    return ESP_OK;
}
