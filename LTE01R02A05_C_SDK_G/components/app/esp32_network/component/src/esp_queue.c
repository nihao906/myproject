...
#include "esp_queue.h"
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "esp_queue", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "esp_queue", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "esp_queue", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "esp_queue", msg, ##__VA_ARGS__)
/* create new node */
...
int esp_queue_put(esp_queue_t* q, void *data)
{
	...
	if (!new_node) {
		LOGI("malloc failed in qpp_q_put\n");
