...
#include "stats.h"
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "stats", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "stats", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "stats", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "stats", msg, ##__VA_ARGS__)
#if TEST_RAW_TP
...
{
	LOGI("ESP peripheral capabilities: 0x%x\n\r", cap);
	if ((cap & ESP_TEST_RAW_TP) == ESP_TEST_RAW_TP) {
		if ((cap & ESP_TEST_RAW_TP__ESP_TO_HOST) == ESP_TEST_RAW_TP__ESP_TO_HOST) {
			...
			LOGI("esp: start testing of ESP->Host raw throughput\n\r");
		} else {
			start_test_raw_tp(ESP_TEST_RAW_TP__TX);
			LOGI("esp: start testing of Host->ESP raw throughput\n\r");
		}
	} else {
		LOGI("esp: stop raw throuput test if running\n");
		stop_test_raw_tp();
	}
