...
#include <errno.h>
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "common", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "common", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "common", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "common", msg, ##__VA_ARGS__)
/** Constants/Macros **/
...
int get_num_from_string(int *val, char *arg)
{
  ...
  if (!arg || (arg[0]=='\0')) {
    LOGI("No number Identified \n");
    return STM_FAIL;
  }

  if (!val) {
    LOGI("No memory allocated \n");
    return STM_FAIL;
  }
