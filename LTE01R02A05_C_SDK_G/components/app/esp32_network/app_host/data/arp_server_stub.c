...
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "arp_server_stub", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "arp_server_stub", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "arp_server_stub", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "arp_server_stub", msg, ##__VA_ARGS__)
...
static void arping_set_ipaddr(uint8_t *stream, ie_type_e type, uint32_t ip)
{
	...
	if (type == IP_ADDR_TYPE_SRC ) {
		...
		LOGI("Address type be either src/dst\n");
	}
}
...
static void arping_set_mac(uint8_t *stream, ie_type_e type, const uint8_t *mac)
{
	if (type == MAC_ADDR_TYPE_SRC) {
		...
		LOGI("mac type be either src/dst/repeatSrc\n");
	}
}
...
static uint8_t * arping_get_mac(uint8_t *stream, ie_type_e type)
{
	...
	if (type == MAC_ADDR_TYPE_SRC) {
		...
		LOGI("mac type be either src/dst/srcRepeat\n");
	}
	return mac;
}
...
static void display_arp(uint8_t *pkt, uint16_t pkt_len, uint8_t arp_msg)
{
	...
	if (arp_msg < MAX_MSG_TYPE_ARPING) {
		...
		QL_LOG(QL_LOG_LEVEL_INFO,"mac_s, 30", "%02x:%02x:%02x:%02x:%02x:%02x",
				mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

		LOGI("0x%05lX %s: %u bytes from %s (%s)\n\r",
