...
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "control", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "control", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "control", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "control", msg, ##__VA_ARGS__)
...
stm_ret_t get_self_ip_station(uint32_t *self_ip)
{
	...
	{
		LOGI("invalid src ip addr from INPUT_STATION_SRC_IP %s\n\r",
...
stm_ret_t get_self_ip_softap(uint32_t *self_ip)
{
	...
	{
		LOGI("invalid src ip addr from INPUT_SOFTAP_SRC_IP %s\n\r",
			   INPUT_SOFTAP_SRC_IP);
		return STM_FAIL;
	}
	return STM_OK;
...
stm_ret_t get_arp_dst_ip_station(uint32_t *sta_ip)
{
	...
	{
		LOGI("invalid src ip addr from INPUT_STATION_ARP_DEST_IP %s\n\r",
...
stm_ret_t get_arp_dst_ip_softap(uint32_t *soft_ip)
{
	...
	{
		LOGI("invalid src ip addr from INPUT_SOFTAP_ARP_DEST_IP %s\n\r",
...
void control_path_init(void (*control_path_evt_handler)(uint8_t))
{
	...
	{
		LOGI("init hosted control lib failed\n\r");
		return;
	}
	...
	ql_task_t SEM_Thread;
	int err = ql_rtos_task_create(&SEM_Thread, CONTROL_PATH_TASK_STACK_SIZE, APP_PRIORITY_BELOW_NORMAL, "control_path_task", control_path_task, NULL, 5);
	// assert(SEM_Thread);
...
static uint8_t get_param_softap_bw(void)
{
	...
	{
		LOGI("%s not supported, possible values: [\"20\"|\"40\"] for INPUT_SOFTAP_BANDWIDTH, Default to \"40\"\n\r",
...
static uint8_t get_param_softap_encryption(void)
{
	...
	LOGI("%s not supported for INPUT_SOFTAP_ENCRYPTION. Default to OPEN\n\r",
...
static void print_configuration_parameters(void)
{
	...
	LOGI("\n\r");
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|           Parameters              |             Values                        |\n\r");
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|      INPUT__OPERATING_MODE        |             %-30s|\n\r", INPUT__OPERATING_MODE);
	LOGI("|      INPUT_GET_AP_SCAN_LIST       |             %-30s|\n\r", INPUT_GET_AP_SCAN_LIST);
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|            SOFTAP                 |                                           |\n\r");
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|        INPUT_SOFTAP__SSID         |             %-30s|\n\r", INPUT_SOFTAP__SSID);
	LOGI("|       INPUT_SOFTAP_PASSWORD       |             %-30s|\n\r", INPUT_SOFTAP_PASSWORD);
	LOGI("|      INPUT_SOFTAP_BANDWIDTH       |             %-30s|\n\r", INPUT_SOFTAP_BANDWIDTH);
	LOGI("|       INPUT_SOFTAP_CHANNEL        |             %-30s|\n\r", INPUT_SOFTAP_CHANNEL);
	LOGI("|      INPUT_SOFTAP_ENCRYPTION      |             %-30s|\n\r", INPUT_SOFTAP_ENCRYPTION);
	LOGI("|      INPUT_SOFTAP_MAX_CONN        |             %-30s|\n\r", INPUT_SOFTAP_MAX_CONN);
	LOGI("|     INPUT_SOFTAP_SSID_HIDDEN      |             %-30s|\n\r", INPUT_SOFTAP_SSID_HIDDEN);
	LOGI("|       INPUT_SOFTAP_SRC_IP         |             %-30s|\n\r", INPUT_SOFTAP_SRC_IP);
	LOGI("|      INPUT_SOFTAP_ARP_DEST_IP     |             %-30s|\n\r", INPUT_SOFTAP_ARP_DEST_IP);
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|            STATION                |                                           |\n\r");
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
	LOGI("|        INPUT_STATION__SSID        |             %-30s|\n\r", INPUT_STATION__SSID);
	LOGI("|       INPUT_STATION_BSSID         |             %-30s|\n\r", INPUT_STATION_BSSID);
	LOGI("|   INPUT_STATION_IS_WPA3_SUPPORTED |             %-30s|\n\r", INPUT_STATION_IS_WPA3_SUPPORTED);
	LOGI("|      INPUT_STATION_PASSWORD       |             %-30s|\n\r", INPUT_STATION_PASSWORD);
	LOGI("|       INPUT_STATION_SRC_IP        |             %-30s|\n\r", INPUT_STATION_SRC_IP);
	LOGI("|     INPUT_STATION_ARP_DEST_IP     |             %-30s|\n\r", INPUT_STATION_ARP_DEST_IP);
	LOGI("+-----------------------------------+-------------------------------------------+\n\r");
}
...
static int station_connect(void)
{
	...
	LOGI("Station mode: ssid: %s passwd %s \n\r",
	...
	{
		LOGI("Could not parse listen_interval, defaulting to 3\n\r");
	...
	{
		LOGI("Failed to set wifi mode to %u\n\r", wifi_mode);
		return STM_FAIL;
	}

	if (test_station_mode_get_mac_addr(mac))
	{
		LOGI("Failed to get wifi mac\n\r");
		return STM_FAIL;
	}
...
static int softap_start(void)
{
	...
	LOGI("SoftAP mode: ssid: %s passwd %s \n\r",
	...
	if (ret)
	{
		LOGI("Wrong input entered for softap channel\n\r");
		LOGI("Setting softap channel to default value 1\n\r");
	...
	if (ret)
	{
		LOGI("Wrong input entered for softap maximum connections\n\r");
		LOGI("Setting softap maximum connections to default value 4\n\r");
	...
	for (;;)
	{
		...
		if (!stop)
		{
			if (get_boolean_param(INPUT_GET_AP_SCAN_LIST) && !scan_ap_list)
			{
				...
				if (ret)
				{
					LOGI("Failed to scan Nearby AP list, anyway continue for rest functionality\n");
					scan_ap_list = true;
					continue;
				}
				scan_ap_list = true;
			...
			case MODE_STATION:
			{
				...
				{
					stop = true;
					LOGI("Maximum retry done to connect with AP\n\r");
				}
				break;
			}
			case MODE_SOFTAP:
			{
				...
				{
					stop = true;
					LOGI("Maximum retry done to start SOFTAP \n\r");
				}
				break;
			}
			...
			default:
			{
				LOGI("Operating mode is not selected.\n\r");
				LOGI("Please revisit Project settings->\n\r");
				LOGI("   ->C/C++ Build->Build Variables->INPUT_OPERATING_MODE\n\r");
				LOGI("=> Either \"STATION\" or \"SOFTAP\" or \"SOFTAP+STATION\"\n\r");
				hard_delay(500000);
			}
			}
		}
