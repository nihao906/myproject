...
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "control_utils", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "control_utils", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "control_utils", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "control_utils", msg, ##__VA_ARGS__)
#define MAX_MAC_STR_LEN   256//tip:未找到定义自添加
/***** Please Read *****/
...
static char * get_timestamp(char *str, uint16_t str_size)
{
	if (str && str_size>=MIN_TIMESTAMP_STR_SIZE) {
		ql_timeval_t timetamp;
		ql_gettimeofday(&timetamp);
		uint64_t t = (uint64_t)timetamp.sec * 1000 + timetamp.usec / 1000;
		snprintf(str, str_size, "%llu", t);
		// time_t t = time(NULL);
		// struct tm tm = *localtime(&t);
		// sLOGI(str, "%d-%02d-%02d %02d:%02d:%02d > ", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
		return str;
	}
	return NULL;
}
...
static void process_failed_responses(ctrl_cmd_t *app_msg)
{
	...
	switch (app_msg->resp_event_status) {
		case CTRL_ERR_REQ_IN_PROG:
			LOGI("Error reported: Command In progress, Please wait\n\r");
			break;
		case CTRL_ERR_REQUEST_TIMEOUT:
			LOGI("Error reported: Response Timeout\n\r");
			break;
		case CTRL_ERR_MEMORY_FAILURE:
			LOGI("Error reported: Memory allocation failed\n\r");
			break;
		case CTRL_ERR_UNSUPPORTED_MSG:
			LOGI("Error reported: Unsupported control msg\n\r");
			break;
		case CTRL_ERR_INCORRECT_ARG:
			LOGI("Error reported: Invalid or out of range parameter values\n\r");
			break;
		case CTRL_ERR_PROTOBUF_ENCODE:
			LOGI("Error reported: Protobuf encode failed\n\r");
			break;
		case CTRL_ERR_PROTOBUF_DECODE:
			LOGI("Error reported: Protobuf decode failed\n\r");
			break;
		case CTRL_ERR_SET_ASYNC_CB:
			LOGI("Error reported: Failed to set aync callback\n\r");
			break;
		case CTRL_ERR_TRANSPORT_SEND:
			LOGI("Error reported: Problem while sending data on serial driver\n\r");
			break;
		default:
	...
	switch (app_msg->msg_id) {
		...
		case CTRL_RESP_OTA_WRITE: {
			...
			if (app_msg->resp_event_status == CTRL_ERR_NO_AP_FOUND) {
				...
				LOGI("Failed to connect with AP\n\r");
			}
			break;
		} case CTRL_RESP_START_SOFTAP: {
			LOGI("Failed to start SoftAP\n\r");
			break;
		}
		case CTRL_RESP_STOP_SOFTAP:
		case CTRL_RESP_GET_SOFTAP_CONFIG: {
			LOGI("Possibly softap is not running/started\n\r");
			break;
		} default: {
			LOGI("Failed Control Response\n\r");
			break;
		}
	}
}
...
int unregister_event_callbacks(void)
{
	...
	for (evt=CTRL_EVENT_BASE+1; evt<CTRL_EVENT_MAX; evt++) {
		if (CALLBACK_SET_SUCCESS != reset_event_callback(evt) ) {
			LOGI("reset event callback failed for event[%u]\n\r", evt);
			ret = FAILURE;
		}
	}
	return ret;
...
int register_event_callbacks(void)
{
	...
	for (evt=0; evt<sizeof(events)/sizeof(event_callback_table_t); evt++) {
		if (CALLBACK_SET_SUCCESS != set_event_callback(events[evt].event, events[evt].fun) ) {
			LOGI("event callback register failed for event[%u]\n\r", events[evt].event);
			ret = FAILURE;
			break;
		}
	}
...
int ctrl_app_resp_callback(ctrl_cmd_t * app_resp)
{
	...
	if (!app_resp || (app_resp->msg_type != CTRL_RESP)) {
		if (app_resp)
			LOGI("Msg type is not response[%u]\n\r",app_resp->msg_type);
		goto fail_resp;
	}

	if ((app_resp->msg_id <= CTRL_RESP_BASE) || (app_resp->msg_id >= CTRL_RESP_MAX)) {
		LOGI("Response Msg ID[%u] is not correct\n\r",app_resp->msg_id);
		goto fail_resp;
	}
	...
	switch(app_resp->msg_id) {
		...
		case CTRL_RESP_GET_MAC_ADDR: {
			LOGI("mac address is %s\n\r", app_resp->u.wifi_mac.mac);
			break;
		} case CTRL_RESP_SET_MAC_ADDRESS : {
			LOGI("MAC address is set\n\r");
			break;
		} case CTRL_RESP_GET_WIFI_MODE : {
			LOGI("wifi mode is : ");
			switch (app_resp->u.wifi_mode.mode) {
				case WIFI_MODE_STA:     LOGI("station\n\r");        break;
				case WIFI_MODE_AP:      LOGI("softap\n\r");         break;
				case WIFI_MODE_APSTA:   LOGI("station+softap\n\r"); break;
				case WIFI_MODE_NONE:    LOGI("none\n\r");           break;
				default:                LOGI("unknown\n\r");        break;
			}
			break;
		} case CTRL_RESP_SET_WIFI_MODE : {
			LOGI("wifi mode is set\n\r");
			break;
			...
			if (!list) {
				LOGI("Failed to get scanned AP list\n\r");
				goto fail_resp;
			} else {

				LOGI("Number of available APs is %d\n\r", w_scan_p->count);
				for (i=0; i<w_scan_p->count; i++) {
					LOGI("%d) ssid \"%s\" bssid \"%s\" rssi \"%d\" channel \"%d\" auth mode \"%d\" \n\r",\
			...
			if (0 == strncmp(SUCCESS_STR, p->status, strlen(SUCCESS_STR))) {
				LOGI("AP's ssid '%s'\n\r", p->ssid);
				LOGI("AP's bssid i.e. MAC address %s\n\r", p->bssid);
				LOGI("AP's channel number %d\n\r", p->channel);
				LOGI("AP's rssi %d\n\r", p->rssi);
				LOGI("AP's encryption mode %d\n\r", p->encryption_mode);
			} else {
				LOGI("Station mode status: %s\n\r",p->status);
			}
			break;
		} case CTRL_RESP_CONNECT_AP : {
			LOGI("Connected\n\r");
			...
			LOGI("softAP ssid %s\n\r", resp_p->ssid);
			LOGI("softAP pwd %s\n\r", resp_p->pwd);
			LOGI("softAP channel ID %d\n\r", resp_p->channel);
			LOGI("softAP encryption mode %d\n\r", resp_p->encryption_mode);
			LOGI("softAP max connections %d\n\r", resp_p->max_connections);
			LOGI("softAP ssid broadcast status %d \n", resp_p->ssid_hidden);
			LOGI("softAP bandwidth mode %d\n\r", resp_p->bandwidth);

			break;
		} case CTRL_RESP_SET_SOFTAP_VND_IE : {
			LOGI("Success in set vendor specific ie\n\r");
			break;
		} case CTRL_RESP_START_SOFTAP : {
			LOGI("ESP softAP started\n\r");
			break;
			...
			LOGI("sta list count: %u\n\r",count);
			if (!count) {
				LOGI("No station found\n\r");
				goto fail_resp;
			}

			if (!stations_list) {
				LOGI("Failed to get connected stations list\n\r");
			} else if (count) {
				for (i=0; i<count; i++) {
					LOGI("%d th stations's bssid \"%s\" rssi \"%d\" \n\r",i, \
							stations_list[i].bssid, stations_list[i].rssi);
				}
			}
			break;
		} case CTRL_RESP_STOP_SOFTAP : {
			LOGI("ESP softAP stopped\n\r");
			break;
		} case CTRL_RESP_SET_PS_MODE : {
			LOGI("Wifi power save mode set\n\r");
			break;
		} case CTRL_RESP_GET_PS_MODE : {
			LOGI("Wifi power save mode is: ");
			...
			LOGI("OTA begin success\n\r");
			break;
		} case CTRL_RESP_OTA_WRITE : {
			LOGI("OTA write success\n\r");
			break;
		} case CTRL_RESP_OTA_END : {
			LOGI("OTA end success\n\r");
			break;
		} case CTRL_RESP_SET_WIFI_MAX_TX_POWER: {
			LOGI("Set wifi max tx power success\n\r");
			break;
		} case CTRL_RESP_GET_WIFI_CURR_TX_POWER: {
			LOGI("wifi curr tx power : %d\n\r",
					app_resp->u.wifi_tx_power.power);
			break;
		} case CTRL_RESP_CONFIG_HEARTBEAT: {
			LOGI("Heartbeat operation successful\n\r");
			break;
		} default: {
			LOGI("Invalid Response[%u] to parse\n\r", app_resp->msg_id);
			break;
		}
	}
...
int test_ota(char* image_path)
{
	...
	if (ret == SUCCESS) {
		...
		if (f == NULL) {
			...
			LOGI("Success in opening %s file \n", image_path);
		}
		while (!feof(f)) {
	...
	LOGI("ESP will restart after 5 sec\n");
	return SUCCESS;
#endif
	LOGI("For OTA, user need to integrate HTTP client lib and then invoke OTA\n\r");
	return FAILURE;
}
