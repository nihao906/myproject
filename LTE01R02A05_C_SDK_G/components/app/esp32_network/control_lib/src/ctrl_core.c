...
#include <unistd.h>
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "ctrl_core", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "ctrl_core", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "ctrl_core", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "ctrl_core", msg, ##__VA_ARGS__)
#ifdef MCU_SYS
#include "common.h"
// #define LOGI(...)      \
// 	LOGI(__VA_ARGS__); \
...
#define CHECK_CTRL_MSG_NON_NULL(msGparaM)    \
	...
	{                                        \
		LOGI("Failed to process rx data\n"); \
		goto fail_parse_ctrl_msg;            \
	}
...
#define CHECK_CTRL_MSG_FAILED(msGparaM)                                                                    \
	...
	{                                                                                                      \
		LOGI("Failure[%d] resp/event: possibly precondition not met\n", (int)app_resp->resp_event_status); \
		goto fail_parse_ctrl_msg;                                                                          \
	}
...
#define CTRL_ALLOC_ASSIGN(TyPe, MsG_StRuCt)                          \
	...
	if (!req_payload)                                                \
	{                                                                \
		LOGI("Failed to allocate memory for req.%s\n", #MsG_StRuCt); \
...
static int convert_mac_to_bytes(uint8_t *out, size_t out_size, char *s)
{
	...
	if (!s || (strlen(s) < MIN_MAC_STR_LEN) || (out_size < MAC_SIZE_BYTES))
	{
		if (!s)
		{
			LOGI("empty input mac str\n");
		}
		else if (strlen(s) < MIN_MAC_STR_LEN)
		{
			LOGI("strlen of in str [%zu]<MIN_MAC_STR_LEN[%u]\n",
				 strlen(s), MIN_MAC_STR_LEN);
		}
		else
		{
			LOGI("out_size[%zu]<MAC_SIZE_BYTES[%u]\n",
...
static int ctrl_app_parse_event(CtrlMsg *ctrl_msg, ctrl_cmd_t *app_ntfy)
{
	...
	{
		LOGI("NULL Ctrl event or App struct\n");
		goto fail_parse_ctrl_msg;
	}
	...
	case CTRL_EVENT_STATION_CONNECTED_TO_AP:
	{
		...
		/*LOGI("EVENT: Station mode: Disconnect with reason [%u]\n",
	...
	case CTRL_EVENT_STATION_CONNECTED_TO_ESP_SOFTAP:
	{
		...
		if (SUCCESS == app_ntfy->resp_event_status)
		{
			...
			/*LOGI("EVENT: SoftAP mode: Disconnect MAC[%s]\n",
	...
	default:
	{
		LOGI("Invalid/unsupported event[%u] received\n", ctrl_msg->msg_id);
		goto fail_parse_ctrl_msg;
		break;
	}
	}
...
static int ctrl_app_parse_resp(CtrlMsg *ctrl_msg, ctrl_cmd_t *app_resp)
{
	...
	{
		LOGI("NULL Ctrl resp or NULL App Resp\n");
	...
	case CTRL_RESP_GET_AP_CONFIG:
	{
		...
		case CTRL_ERR_NOT_CONNECTED:
			...
			LOGI("Station is not connected to AP \n");
			goto fail_parse_ctrl_msg;
			break;
	...
	case CTRL_RESP_SET_WIFI_MAX_TX_POWER:
	{
		...
		{
		case FAILURE:
			LOGI("Failed to set max tx power\n");
			goto fail_parse_ctrl_msg;
			break;
		case SUCCESS:
			break;
		case CTRL_ERR_OUT_OF_RANGE:
			LOGI("Power is OutOfRange. Check api doc for reference\n");
			goto fail_parse_ctrl_msg;
			break;
		default:
			LOGI("unexpected response\n");
			goto fail_parse_ctrl_msg;
			break;
		}
		break;
	...
	default:
	{
		LOGI("Unsupported Control Resp[%u]\n", ctrl_msg->msg_id);
		goto fail_parse_ctrl_msg;
		break;
	}
	}
...
static int is_event_callback_registered(int event)
{
	...
	{
		LOGI("Could not identify event[%u]\n", event);
...
static int process_ctrl_rx_msg(CtrlMsg *proto_msg, ctrl_rx_ind_t ctrl_rx_func)
{
	...
	if (proto_msg->msg_type == CTRL_MSG_TYPE__Event)
	{
			...
			is_event_callback_registered(proto_msg->msg_id))
		{
			...
			if (!app_event)
			{
				LOGI("Failed to allocate app_event\n");
				goto free_buffers;
			}
	...
	else if (proto_msg->msg_type == CTRL_MSG_TYPE__Resp)
	{
		...
		if (!app_resp)
		{
			LOGI("Failed to allocate app_resp\n");
			goto free_buffers;
		}
	...
	else
	{
		...
		LOGI("Incorrect Ctrl Msg Type[%u]\n", proto_msg->msg_type);
		goto free_buffers;
	}
	return SUCCESS;
...
static void ctrl_rx_thread(void const *arg)
{
	...
	if (!ctrl_rx_func)
	{
		LOGI("ERROR: NULL rx async cb for esp_queue,sem\n");
		return;
	}
	...
	if (!ctrl_msg_Q)
	{
		LOGI("Ctrl msg Q is not created\n");
		return;
	}
...
static int spawn_ctrl_rx_thread(void)
{
	...
	{
		LOGI("Thread creation failed for ctrl_rx_thread\n");
		return FAILURE;
	}
	return SUCCESS;
}
...
static ctrl_cmd_t *get_response(int *read_len, int timeout_sec)
{
	...
	if (ret)
	{
		if (errno == ETIMEDOUT)
		{
			LOGI("Control response timed out after %u sec\n", timeout_sec);
		}

		else
		{
			LOGI("ctrl lib error[%u] in sem of timeout[%u]\n", errno, timeout_sec);
		}
	...
	{
		LOGI("Ctrl Q empty or uninitialised\n");
		return NULL;
	}
...
static int set_async_resp_callback(int req_msg_id, ctrl_resp_cb_t resp_cb)
{
	...
	{
		LOGI("Not able to map new request to resp id\n");
		return MSG_ID_OUT_OF_ORDER;
	}
	else
	{
...
int is_async_resp_callback_registered(ctrl_cmd_t req)
{
	...
	if (exp_resp_msg_id >= CTRL_RESP_MAX)
	{
		LOGI("Not able to map new request to resp id, using sync path\n");
		return MSG_ID_OUT_OF_ORDER;
	}
...
static void ctrl_async_timeout_handler(void const *arg)
{
	...
	if (!func)
	{
		LOGI("NULL func, failed to call callback\n");
		hosted_post_semaphore(ctrl_req_sem);
		return;
	}
	ctrl_cmd_t *app_resp = NULL;
...
int ctrl_app_send_req(ctrl_cmd_t *app_req)
{
	...
	case CTRL_REQ_SET_SOFTAP_VND_IE:
	{
		...
		{
			LOGI("Invalid vendor ie ID index \n");
			failure_status = CTRL_ERR_INCORRECT_ARG;
			goto fail_req;
		}
	...
	case CTRL_REQ_START_SOFTAP:
	{
			...
			(p->encryption_mode > WIFI_AUTH_WPA_WPA2_PSK))
		{
			...
			LOGI("Asked Encryption mode not supported\n");
			failure_status = CTRL_ERR_INCORRECT_ARG;
			goto fail_req;
		}
		...
		{
			LOGI("Invalid maximum connection number\n");
			failure_status = CTRL_ERR_INCORRECT_ARG;
			goto fail_req;
		}
	...
	case CTRL_REQ_CONFIG_HEARTBEAT:
	{
		...
		{
			LOGI("Enable heartbeat with duration %ld\n", (long int)req_payload->duration);
			if (CALLBACK_AVAILABLE != is_event_callback_registered(CTRL_EVENT_HEARTBEAT))
				LOGI("Note: ** Subscribe heartbeat event to get notification **\n");
		}
		else
		{
			LOGI("Disable Heartbeat\n");
		}
		break;
	}
	...
	if (!tx_data)
	{
		LOGI("Failed to allocate memory for tx_data\n");
		failure_status = CTRL_ERR_MEMORY_FAILURE;
		goto fail_req;
	}
	...
	if (ret < 0)
	{
		LOGI("could not set callback for req[%u]\n", req.msg_id);
	...
	if (app_req->ctrl_resp_cb)
	{
		...
		if (!async_timer_handle)
		{
			LOGI("Failed to start async resp timer\n");
			goto fail_req;
		}
	...
	{
		LOGI("Send control req[%u] failed\n", req.msg_id);
...
int deinit_hosted_control_lib_internal(void)
{
	...
	{
		ret = FAILURE;
		LOGI("ctrl req sem deinit failed\n");
	}

	if (read_sem && hosted_destroy_semaphore(read_sem))
	{
		ret = FAILURE;
		LOGI("read sem deinit failed\n");
	}
	...
	{
		ret = FAILURE;
		LOGI("Serial de-init failed\n");
	}

	if (ctrl_rx_thread_handle && cancel_ctrl_rx_thread())
	{
		ret = FAILURE;
		LOGI("cancel ctrl rx thread failed\n");
	}
...
int init_hosted_control_lib_internal(void)
{
	...
	if (getuid())
	{
		LOGI("Please re-run program with superuser access\n");
		return FAILURE;
	}
#endif
	...
	if (!ctrl_msg_Q)
	{
		LOGI("Failed to create app ctrl msg Q\n");
		goto free_bufs;
	}
...
int interface_up(int sockfd, char *iface)
{
	...
	{
		LOGI("Failed: Max interface len allowed: %zu \n", sizeof(req.ifr_name) - 1);
		return FAILURE;
	}
...
int set_hw_addr(int sockfd, char *iface, char *mac)
{
	...
	if (ret)
	{
		LOGI("Failed to convert mac address \n");
		return FAILURE;
	}
