...
// #include "projdefs.h"
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "netdev_stub", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "netdev_stub", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "netdev_stub", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "netdev_stub", msg, ##__VA_ARGS__)
...
int netdev_open(netdev_handle_t ndev)
{
	...
	if (ndev->rx_q) {
		ql_rtos_queue_delete(ndev->rx_q);
		return STM_OK;
	}

	int ret = ql_rtos_queue_create(&ndev->rx_q,RX_QUEUE_SIZE, sizeof(struct pbuf));
...
netdev_handle_t netdev_alloc(uint32_t sizeof_priv, char *name)
{
	...
	if (ndev) {
		...
		if (!ndev->priv) {
			LOGI("Failed to allocate memory for priv\n");
			free(ndev);
			ndev = NULL;
			return NULL;
		}
	} else {
		LOGI("Failed to allocate memory for net dev\n");
	}
...
int netdev_rx(netdev_handle_t dev, struct pbuf *net_buf)
{
	...
	if (ndev->state == NETDEV_STATE_UP) {
        // if (p != osiEventSend(ndev->rx_q, net_buf)){
		if (true != ql_rtos_queue_release(ndev->rx_q,sizeof(netdev_handle_t), &net_buf, QL_WAIT_FOREVER)) {
			LOGI ("Failed to enqueue received packet\n");
			goto done;
		}
