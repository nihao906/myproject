...
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "netdev_api", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "netdev_api", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "netdev_api", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "netdev_api", msg, ##__VA_ARGS__)
/**
...
struct pbuf * network_read(struct network_handle *handle, uint32 xTicksToWait)
{
	...
	ql_rtos_queue_wait(handle->ndev->rx_q, buffer,sizeof(buffer) ,xTicksToWait);
	ql_rtos_queue_get_cnt(handle->ndev->rx_q,&buffer);
