...
#include "trace.h"
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "serial_ll_if", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "serial_ll_if", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "serial_ll_if", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "serial_ll_if", msg, ##__VA_ARGS__)
...
static ql_queue_t to_serial_ll_intf_queue[MAX_SERIAL_INTF];
...
{
	if (! serial_ll_hdl) {
		LOGI("serial invalid hdr\n\r");
		return STM_FAIL;
	}

	if (serial_ll_hdl->queue) {
		/* clean up earlier queue */
		ql_rtos_queue_delete(serial_ll_hdl->queue);
	}

	/* Queue - serial rx */
	int ret = ql_rtos_queue_create(&serial_ll_hdl->queue,sizeof(interface_buffer_handle_t),
							 ...
							 uint16_t * rlen)
{
	...
	*rlen = 0 ;
	 ...
	 */
	if (true != ql_rtos_queue_wait(serial_ll_hdl->queue, &buf_handle,sizeof(buf_handle), QL_WAIT_FOREVER)) {
		LOGI("serial queue recv failed \n\r");
		return NULL;
	}
	...
	uint8_t * wbuffer, const uint16_t wlen)
{
	...
	if ((! serial_ll_hdl) || (serial_ll_hdl->state != ACTIVE)) {
		LOGI("serial invalid interface for write\n\r");
		return STM_FAIL;
	}
...
#define SERIAL_ALLOC_REALLOC_RDATA() \
	do { \
		...
		if (!r.data) { \
			LOGI("Failed to allocate serial data\n\r"); \
	...
	if ((! serial_ll_hdl) || (serial_ll_hdl->state != ACTIVE)) {
		LOGI("Serial interface not registered yet\n\r");
		goto serial_buff_cleanup;
	}
	...
	new_buf_handle.free_buf_handle = clear;
	// if(){}
	...
	if (true != ql_rtos_queue_release(serial_ll_hdl->queue,sizeof(interface_buffer_handle_t),
		    &new_buf_handle, QL_WAIT_FOREVER)) {
		LOGI("Failed send serialif queue[%u]\n\r", new_buf_handle.if_num);
		goto serial_buff_cleanup;
	}
...
serial_ll_handle_t * serial_ll_init(void(*serial_rx_callback)(void))
{
	...
	if ((conn_num+1) < MAX_SERIAL_INTF) {
		...
		if (! serial_ll_hdl) {
			LOGI("Serial interface - malloc failed\n\r");
			return NULL;
		}
		...
		LOGI("Number of serial interface connections overflow\n\r");
		return NULL;
	}
