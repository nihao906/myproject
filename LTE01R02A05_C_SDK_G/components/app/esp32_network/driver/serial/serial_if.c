...
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "serial_if", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "serial_if", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "serial_if", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "serial_if", msg, ##__VA_ARGS__)
/** Constants/Macros **/
#define SUCCESS                           0
#define FAILURE                          -1
...
#ifdef MCU_SYS
#define command_log(format, ...)          LOGI(format "\r", ##__VA_ARGS__);
#else
#define command_log(...)                  LOGI("%s:%u ",__func__,__LINE__); \
                                          LOGI(__VA_ARGS__);
#endif
...
int transport_pserial_open(void)
{
	...
	if (serial_handle) {
		LOGI("Already opened returned\n");
		return ret;
	}

	serial_handle = serial_drv_open(transport);
	if (!serial_handle) {
		LOGI("serial interface open failed, Is the driver loaded?\n");
		return FAILURE;
	}

	ret = control_path_platform_init();
	if (ret != SUCCESS) {
		LOGI("Platform init failed\n");
