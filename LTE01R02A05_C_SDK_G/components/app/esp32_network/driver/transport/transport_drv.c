...
#include "stats.h"
#include "ql_log.h"
#define LOGD(msg, ...) QL_LOG(QL_LOG_LEVEL_DEBUG, "transport_drv", msg, ##__VA_ARGS__)
#define LOGI(msg, ...) QL_LOG(QL_LOG_LEVEL_INFO, "transport_drv", msg, ##__VA_ARGS__)
#define LOGW(msg, ...) QL_LOG(QL_LOG_LEVEL_WARN, "transport_drv", msg, ##__VA_ARGS__)
#define LOGE(msg, ...) QL_LOG(QL_LOG_LEVEL_ERROR, "transport_drv", msg, ##__VA_ARGS__)
...
int process_init_event(uint8_t *evt_buf, uint8_t len)
{
	...
	while (len_left) {
		...
		if (*pos == ESP_PRIV_CAPABILITY) {
			...
			LOGI("priv test raw tp\n\r");
#if TEST_RAW_TP
			process_test_capabilities(*(pos + 2));
#endif
		} else {
			LOGI("Unsupported tag in event\n\r");
		}
		pos += (tag_len+2);
		len_left -= (tag_len+2);
	}
	...
	if ((chip_type != ESP_PRIV_FIRMWARE_CHIP_ESP32) &&
		...
		LOGI("ESP board type is not mentioned, ignoring [%d]\n\r", chip_type);
		chip_type = ESP_PRIV_FIRMWARE_CHIP_UNRECOGNIZED;
		return -1;
	} else {
		LOGI("ESP board type is : %d \n\r", chip_type);
	}
