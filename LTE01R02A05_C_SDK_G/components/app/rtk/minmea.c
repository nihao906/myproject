...
#define next_field()                            \
    ...
    while (*format)
    {
        ...
        break;
        case 'F':
        { // Fractional value with scale (struct minmea_double).
            int sign = 0;
...
uint64_t minmea_coord_scale(struct minmea_double *f, int scale)
{
    if (f->scale == 0 || scale == 0)
        return 0;
    uint64_t degrees = f->value / (f->scale * 100ll);
    uint64_t minutes = f->value % (f->scale * 100ll);
    uint64_t degree = degrees * scale;
    if (scale >= f->scale)
    {
        degree += minutes * (scale / f->scale) / 60;
    }
    else
    {
        degree += minutes / (f->scale / scale) / 60;
    }
    return degree;
}
