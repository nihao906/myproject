...
#define LOGI(msg, ...)          QL_LOG(QL_GPIODEMO_LOG_LEVEL, "protocol_uart", msg, ##__VA_ARGS__)
...
esp_loader_error_t loader_initialize_conn(esp_loader_connect_args_t *connect_args)
{
    ...
    LOGI("loader_initialize_conn() trials=%d", trials);

    do {
        LOGI("trials=%d", trials);
        loader_port_start_timer(connect_args->sync_timeout);
        LOGI("call loader_sync_cmd");
        err = loader_sync_cmd();
        LOGI("loader_sync_cmd err:%d", err);
    ...
    LOGI("loader_initialize_conn err:%d", err);
...
static esp_loader_error_t check_response(command_t cmd, uint32_t *reg_value, void *resp, uint32_t resp_size)
{
    ...
    do {
        ...
        if (err != ESP_LOADER_SUCCESS) {
            LOGI("SLIP_receive_packet err:%d", err);
            return err;
        }
