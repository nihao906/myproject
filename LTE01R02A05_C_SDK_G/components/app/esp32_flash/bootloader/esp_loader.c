...
#define LOGI(msg, ...)          QL_LOG(QL_GPIODEMO_LOG_LEVEL, "esp_loader", msg, ##__VA_ARGS__)
...
esp_loader_error_t esp_loader_flash_start(uint32_t offset, uint32_t image_size, uint32_t block_size)
{
    ...
    if (detect_flash_size(&flash_size) == ESP_LOADER_SUCCESS) {
        ...
        LOGI("Flash size detection failed, falling back to default");
    }
...
esp_loader_error_t esp_loader_flash_verify(void)
{
    ...
    if (!md5_match) {
        ...
        LOGI("Error: MD5 checksum does not match:");
        LOGI("Expected:");
        LOGI("%s", (char *)received_md5);
        LOGI("Actual:");
        LOGI("%s", (char *)hex_md5);
