...
>在setting/chips/8***/bsdump.xml文件中支持的profile，可用--profile list列出所有支持的profile项
...
For example:
    ...
    $ dtools.exe heapreport --type list
      supported types:
         8910AP, 8910CP, 8811, 8821, 8850AP, 8850CP
...
Options:
    ...
    --padding <padding>    padding 0/1/-1, default to 0
    --img                  output image file
...
For example:
    ...
    $ dtools.exe mksimage --platform list //通过该命令查看支持的platform
      supported platforms:
         8811, 8850, 8821
    $ dtools.exe mksimage --check list //通过该命令查看支持的check method
...
# simage sign tools
 > 8811/8821/8850处理对象为simage，通过ssign进行签名
...
Options:
    ...
    --anti-rollback                    get version only from pac
    --secure-version                   get secure version from pac
...
For example:
    ...
    $ dtools.exe fotacreate2 --pac \
                 ./pac/v2_ext_merge.pac,./pac/v2_ext_merge2.pac,./setting/fota8910.xml \
                 ./pacmerge_ext.pac --anti-rollback
    //当使用“anti-rollback”这一option是，应对生成的pacmerge_ext.pac文件进行签名操作；
    //还应确保待升级应将开启了secure boot和secure version
...
## fconthash
* calculate cont hash for a fota package.
* Usage: dtools.exe [options] fconthash root_pub  keycert_version input output
* the fota package should be exist.

Options:

    -r, --resign             clear existed certs
    -f, --force              overwite existed signed pack
    --pkcsv15                use PKCS#1 v1.5 for rsa
    --debug_pub <debug_pub>  debug cert public key, in pem format
...
Arguments:
    ...
    root_pub                 file for root public key, in pem format
    keycert_version          security version in key cert
    input                    input unsigned image file
    output                   conthash file
For example:

    $ dtools.exe fconthash MF_PUK_CD.pem 1 --pkcsv15 fota.pack hash.bin.

## fsignbysig
* sign a fota package,no private key,but has signature file.
* Usage: dtools.exe [options] fsignbysig signature root_pub keycert_version input output
* This is to sign a fota package. The existed fota package must be unsigned.
* Unless --force is specified, this will report error if the output
image exists.
* keycert version is not used in current version.
* Unless --resign is specified, this will report error is the input
file is already signed. At resign, the existed certs will be cleared.
...
Arguments:
    ...
    signature                signature file, in hex format
    root_pub                 file for root public key, in pem format
    keycert_version          security version in key cert
    input                    input unsigned fota pack file
    output                   output signed fota pack file

For example:

    $ dtools.exe fsignbysig signature.bin pubkey.pem --pkcsv15\
                 1  fota.pack fota.sign.pack

## fsiginfo
* list fota signature information

Arguments:

    img                  input signed fota file
For example:

    $ dtools.exe fsiginfo 
    fota signature key cert size:           1152
    verify:           OK
    cert type:        1
    pka type:         2 (RSA2048 PKCS#1 v2.1)
    hash type:        0 (SHA256)
    cert size:        1152
    security version: 2
    pubkey hash:      f0bc42c1b29ce9777263dc0c03e3917b0bc5a8871198d84982c3168029e4b3a2
    data size:        352
...
Options:
    ...
    -p, --platform <platform>  platform, 8910/8811/8821/8850
...
* Use lz4 algorithm to compress one file.

Arguments:

    input               input file
    output              output .lz4 file

For example:

    $ dtools.exe lz4zip nvitem.bin nvitem.bin.lz4
...
For example：
    ...
    $ dtools.exe lz4unzip nvitem.bin.lz4 nvitem.bin
...
supported modes:
    8910AP, 8910CP, 8910APLive, 8910CPLive, 8850AP, 8850CP, 8850APLive, 8850CPLive, 8811, 8811Live, 8821, 8821Live
...
## sconthash
* calculate cont hash for a simage.
* Usage: dtools.exe [options] sconthash root_pub  keycert_version input output
* the image should be exist.
...
For example:
    ...
    $ dtools.exe sconthash MF_PUK_CD.pem 1 --pkcsv15 boot.img conthash.bin.

## ssignbysig
* sign an image,no private key,but has signature file.
* Usage: dtools.exe [options] ssignbysig signature root_pub keycert_version input output
* This is to sign an image. The existed inputimage must be unsigned.
* Unless --force is specified, this will report error if the output
image exists.
* keycert version will be checked for anti-rollback. So be careful
...
For example:
    ...
    $ dtools.exe ssignbysig signature.bin pubkey.pem --pkcsv15\
                 1  boot.img boot.sign.img
